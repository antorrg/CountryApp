...
export default class ExperienceService extends BaseService{
  constructor (model, useImages = false, deleteImages = null,parserFunction = null, modelName = '', emptyObject, useMedia = false, deleteMedia = null,  ){
    super(model, useImages, deleteImages, parserFunction, modelName, emptyObject)
    this.useMedia = useMedia
    this.deleteMedia = deleteMedia
  }
  
  async getFeedByCountry (countryId, limit){
    const experiences = await ExperiencesHelper.getFeedByCountry(this.model, countryId, limit)
    if(experiences.length ===0 ) return [this.emptyObject]
    return experiences
  }
  async getPopularOfMonth (countryId, month, year, limit){
    const experiences = await ExperiencesHelper.getPopularOfMonth(this.model, countryId, month, year, limit)
     if(experiences.length ===0 ) return [this.emptyObject]
    return experiences
  }
